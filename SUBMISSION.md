...
**Vetting is evidence, not vibes.** Every applicant is scored with a verified record attached — their on-chain rating, jobs completed, disputes, and whether the portfolio link they gave actually resolves — gathered by Patron and kept separate from the letter they wrote about themselves. When the record and the claim disagree, the record wins. Prompt-injection attempts in cover letters are caught and scored near zero. Ties are broken by on-chain reputation, and the reason is written to the ledger rather than left looking like a coin toss.
